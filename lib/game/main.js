...
.defines(function(){
	...
	StoryGame = ig.Game.extend({
		...
		init: function() {
            ...
            this.buttons = new ig.TouchButtonCollection([
                ...
                new ig.TouchButton( 'settings', {left: 45, top: 50}, 40, 40, this.buttonSettingsImage, 0 ),
                new ig.TouchButton( 'right', {right: 45, bottom: 50}, 40, 40, this.blackButtonNextImage, 0 ),
                new ig.TouchButton( 'settings', {left: 45, top: 50}, 40, 40, this.buttonWhiteSettingsImage, 0 ),
                new ig.TouchButton( 'left', {left: 45, bottom: 50}, 40, 40, this.whiteButtonBackImage, 0 ),
                new ig.TouchButton( 'restart', {right: 45, bottom: 50}, 40, 40, this.buttonRestartImage, 0 ),
	...
	PaintGame = ig.Game.extend({
		...
		init: function(){
			...
			var red=new ig.CheckTouchButton( "red", {left:50, bottom: 12}, 92, 92, this.buttonRedImage,1,0,true);
			var orange=new ig.CheckTouchButton( "orange", {left:142, bottom: 12}, 92, 92, this.buttonOrangeImage,1,0,false);
			var yellow=new ig.CheckTouchButton( "yellow", {left:234, bottom: 12}, 92, 92, this.buttonYellowImage,1,0,false);
			var green=new ig.CheckTouchButton( "green", {left:326, bottom: 12}, 92, 92, this.buttonGreenImage,1,0,false);
			var blue=new ig.CheckTouchButton( "blue", {left:418, bottom: 12}, 92, 92, this.buttonBlueImage,1,0,false);
			var indigo=new ig.CheckTouchButton( "indigo", {left:510, bottom: 12}, 92, 92, this.buttonIndigoImage,1,0,false);
			var violet=new ig.CheckTouchButton( "violet", {left:602, bottom: 12}, 92, 92, this.buttonVioletImage,1,0,false);
			var eraser=new ig.CheckTouchButton( "eraser", {left:694, bottom: 12}, 92, 92, this.buttonEraserImage,1,0,false);
			...
			this.buttons = new ig.TouchButtonCollection([
                ...
                new ig.TouchButton( 'left', {right: 45, bottom: 50}, 40, 40, this.buttonRestartImage, 0 ),
		...
		update: function(){
			...
			if(this.currentLevel!==0 && ig.input.pressed('left')){
				...
				if(window.Ejecta){
					...
					social.post('twitter', '#colores', 'http://xuaps.com/products/colores', null);
				}
				...
				if(window.Ejecta){
					...
					social.gameWidth=ig.system.realWidth;
					social.gameHeight=ig.system.realHeight;
					social.post('facebook', '#colores', 'http://xuaps.com/products/colores', null);
				}
	...
	SettingsGame = ig.Game.extend({
		...
		init: function(){
			...
			this.buttons = new ig.TouchButtonCollection([
				...
				new ig.TouchButton( 'cancelar', {left:45, bottom: 50}, 40, 40, this.buttonCancelImage,0 ),
				]);
		...
		draw: function(){
			...
			ig.system.context.font = '19pt Garamond-Bold';
			ig.system.context.fillText(i18n.t("LevelSettings.cancelar"), 100, 705);
			ig.system.context.textAlign = 'right';
	...
	CreditsGame = ig.Game.extend({
		...
		init: function(){
			...
			this.buttons = new ig.TouchButtonCollection([
					...
					new ig.TouchButton('musica', {left:100, top: 470}, 400, 30),
				]);
		...
		update: function(){
			...
			if(ig.input.pressed('aceptar')){
				...
				this.openUrl('mailto:tales@xuaps.com?Subject=Colores!');
			}else if(ig.input.pressed('twitter_david')){
				this.openUrl('http://twitter.com/xuapslabs');
			}else if(ig.input.pressed('www_carlos')){
				this.openUrl('http://www.comendador.es');
			}else if(ig.input.pressed('mail_carlos')){
				this.openUrl('mailto:carloscomendador@gmail.com?Subject=Colores!');
		...
		draw: function(){
			...
			ig.system.context.fillText("Carlos Comendador", 400, 185);
			ig.system.context.fillText("Irene Doval", 768, 185);
			
			ig.system.context.font = '19pt Garamond-Bold';
			this.print(i18n.t("david"), 100, 285, 30);
			this.print(i18n.t("carlos"), 380, 285, 30);
			this.print(i18n.t("irene"), 748, 285, 30);
			this.print(i18n.t("musica"), 100, 500, 30);
	...
	i18n.init({ resStore: ig.locales, lng: ig.settings.language}, function(){
		...
		ig.main( '#canvas', HomeGame, 60, 1024, 768,  1, ig.ImpactSplashLoader);
	});
});
