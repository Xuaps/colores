...
.defines(function(){
	...
	PaintGame = ig.Game.extend({
		...
		buttonRestartImage: new ig.Image('media/boton_reiniciar_negro.png'),
		parentalImage: new ig.Image('media/fondo_parental_gate.jpg'),
		buttons:null,
		whiteboard:null,
		clearColor: '#FFFFFF',
		...
		init: function(){
            ...
            this.gate = new ParentalGate(this.parentalImage);
		},
		...
		update: function(){
			...
			if(this.currentLevel!==0 && ig.input.pressed('left')){
				...
				if(window.Ejecta){
					this.gate.askPermissionFor(function(){
	...
	CreditsGame = ig.Game.extend({
		...
		update: function(){
			...
			if(ig.input.pressed('aceptar')){
				...
				this.gate.askPermissionFor(function(){
					this.openUrl('mailto:carlos@comendador.es?Subject='+i18n.t("Home.titulo"));
				}.bind(this));
