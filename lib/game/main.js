	...
	PaintGame = ig.Game.extend({
		...
		init: function(){
			...
			this.buttons = new ig.TouchButtonCollection([
                ...
                new ig.TouchButton( 'home', {right: 45, bottom: 50}, 40, 40, this.buttonRestartImage, 0 ),
		...
		update: function(){
			...
			if(this.currentLevel!==0 && ig.input.pressed('home') && !this.gate.isAskingForPermission()){
				ig.system.setGame(HomeGame);
			}else if(ig.input.pressed('settings') && !this.gate.isAskingForPermission()){
				ig.system.setGame(SettingsGame);
	...
	CreditsGame = ig.Game.extend({
		...
		init: function(){
			...
			this.buttons = new ig.TouchButtonCollection([
					...
					new ig.TouchButton('www_musica', {left:100, top: 470}, 400, 30),
				]);
