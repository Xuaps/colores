...
.defines(function(){
	HomeGame = ig.Game.extend({
		...
		draw: function() {
			...
			ig.system.context.fillText(i18n.t("Home.titulo"), 512, 360-vertical_offset);
			ig.system.context.font = '19pt Garamond-Bold';
			ig.system.context.fillText(i18n.t("Home.subtitulo"), 512, 395-vertical_offset);
			ig.system.context.fillStyle="#FFFFFF";
			ig.system.context.textAlign = 'left';
			ig.system.context.fillText(i18n.t("Home.prefrencias"), 90, 80);
			ig.utils.print(i18n.t("Home.info"), {left:90, bottom:35}, 0);
			ig.system.context.textAlign = 'right';
			ig.utils.print(i18n.t("Home.comenzar"), {left: 930, bottom:35}, 0);
	...
	StoryGame = ig.Game.extend({
		...
		draw: function() {
			...
			if(this.levels[this.currentLevel].name==="LevelRespuesta"){
				ig.system.context.fillStyle="#000000";
				ig.utils.print(i18n.t(this.levels[this.currentLevel].name+".correcto"), {left: 768, bottom: 100}, 30 );
			}else if(this.levels[this.currentLevel].name==="LevelTodos"){
				ig.system.context.fillStyle="#FFFFFF";
				ig.utils.print(i18n.t(this.levels[this.currentLevel].name+".amigos"), {left: 256, top:200}, 30 );
				ig.utils.print(i18n.t(this.levels[this.currentLevel].name+".pregunta"), {left:256, top:340}, 30 );
			}else{
				ig.system.context.fillStyle="#000000";
				ig.utils.print(i18n.t(this.levels[this.currentLevel].name+".presentacion"), {left:256, bottom:40}, 30);
				ig.system.context.fillStyle="#FFFFFF";
				if(this.levels[this.currentLevel].name!=="LevelAmarillo"){
					ig.utils.print(i18n.t(this.levels[this.currentLevel].name+".frase"), {left:768,
					top:40}, 30);
				}else{
					ig.utils.print(i18n.t(this.levels[this.currentLevel].name+".frase"), {left:768,
					bottom:40}, 30);
	...
	PaintGame = ig.Game.extend({
		...
		border: new ig.Image('media/borde.jpg'),
		anillas: new ig.Image('media/anillas.png'),
		buttons:null,
		whiteboard:null,
		clearColor: '#FFFFFF',
		...
		draw: function(){
			this.parent();
			this.anillas.draw(0,0, 0,0, 1024,157);
			this.border.draw(0,ig.system.realHeight-107, 0,0, 1024,107);
			this.buttons.buttons[0].draw();
			this.buttons.buttons[1].draw();
			this.buttons.buttons[2].draw();
			this.buttons.buttons[3].draw();
	...
	SettingsGame = ig.Game.extend({
		...
		buttonOkImage: new ig.Image('media/flecha_izquierda_blanca.png'),
		...
		init: function(){
			...
			this.buttons = new ig.TouchButtonCollection([
				...
				new ig.TouchButton( 'aceptar', {left: 45, top: 50}, 40, 40, this.buttonOkImage,0)
				]);
	...
	CreditsGame = ig.Game.extend({
		...
		init: function(){
			...
			this.buttons = new ig.TouchButtonCollection([
					new ig.TouchButton( 'aceptar', {left: 45, top: 50}, 40, 40, this.buttonOkImage,0),
					new ig.TouchButton('www_david', {left:100, top: 230}, 40, 40, this.buttonWWWImage,0),
					new ig.TouchButton('mail_david', {left:145, top: 230}, 40, 40, this.buttonMailImage,0),
					new ig.TouchButton('twitter_david', {left:190, top: 230}, 40, 40, this.buttonTwitterImage,0),
					new ig.TouchButton('www_carlos', {left:380, top: 230}, 40, 40, this.buttonWWWImage,0),
					new ig.TouchButton('mail_carlos', {left:425, top: 230}, 40, 40, this.buttonMailImage,0),
					new ig.TouchButton('twitter_carlos', {left:470, top: 230}, 40, 40, this.buttonTwitterImage,0),
					new ig.TouchButton('www_irene', {left:748, top: 230}, 40, 40, this.buttonWWWImage,0),
					new ig.TouchButton('mail_irene', {left:793, top: 230}, 40, 40, this.buttonMailImage,0),
		...
		draw: function(){
			...
			ig.system.context.font = '19pt Garamond-Bold';
			this.print(i18n.t("david"), 100, 305, 30);
			this.print(i18n.t("carlos"), 380, 305, 30);
			this.print(i18n.t("irene"), 748, 305, 30);
	...
	var vertical_offset = 0; //  (768-(window.innerHeight/scale))/2
	...
	i18n.init({ resStore: ig.locales, lng: ig.settings.language}, function(){
		...
		var height = Math.min(window.innerHeight, (window.innerWidth * originalHeight) / originalWidth);
		...
		window.addEventListener('resize', function(){
			...
			canvas.style.width = window.innerWidth + "px";
			canvas.style.height = height + "px";
			ig.system.resize( originalWidth, height / scale );
		});

		ig.main( '#canvas', HomeGame, 60, originalWidth, height/scale,  1, ig.ImpactSplashLoader);
